...
export class BrowserStream {
  ...
  private async handleMessage(msg: Record<string, unknown>): Promise<void> {
    ...
    if (type === "pubkey") {
      ...
      await this.e2ee.deriveSecret(msg.key as string);
    ...
    if (type === "encrypted") {
      ...
      try {
        const plaintext = await this.e2ee.decrypt(msg.payload as { iv: string; ciphertext: string; tag: string });
        const decrypted = JSON.parse(plaintext) as Record<string, unknown>;
        debug("decrypted: %s", decrypted.type);
        await this.handleMessage(decrypted);
      } catch (err) {
