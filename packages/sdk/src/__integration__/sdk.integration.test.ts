...
describe.skipIf(!apiKey)("SDK integration tests (live API)", () => {
  let authloop: AuthLoop;

  beforeAll(() => {
    authloop = new AuthLoop({
      apiKey: apiKey!,
  ...
  it("creates a session and gets back all expected fields", async () => {
    ...
    // Clean up
    await authloop.cancelSession(session.sessionId);
  });
  ...
  it("polls session status and sees PENDING", async () => {
    ...
    const status = await authloop.getSession(session.sessionId);
  ...
  it("cancels a session", async () => {
    ...
    await expect(authloop.cancelSession(session.sessionId)).resolves.not.toThrow();
  });

  it("returns 404 when getting a non-existent session", async () => {
    try {
      await authloop.getSession("nonexistent_session_id");
  ...
  it("rejects or no-ops when resolving an already-cancelled session", async () => {
    ...
    try {
      await authloop.resolveSession(session.sessionId);
  ...
  it("waitForResolution returns immediately on CANCELLED", async () => {
    ...
    const status = await authloop.waitForResolution(session.sessionId, { pollInterval: 100 });
    expect(status.status).toBe("CANCELLED");
  });

  it("rejects with 401 on bad API key", async () => {
    const badAuthLoop = new AuthLoop({
      apiKey: "al_live_invalid_key",
      baseUrl: process.env.AUTHLOOP_BASE_URL,
    });

    try {
      await badAuthLoop.toHuman({ service: "Test", cdpUrl: "ws://x" });
      expect.fail("should have thrown");
    } catch (e) {
      expect(e).toBeInstanceOf(AuthLoopError);
